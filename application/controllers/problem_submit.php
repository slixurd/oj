...
class Problem_submit extends CI_Controller {
	...
	public function submit(){
		...
		else if(isset($_POST['problemId']) && isset($_POST['code']) 
			&& isset($_POST['language']) && is_numeric($problemId)){
		//判断用户是否传递了最基本的元素
			...
			if(isset($_POST['contest_id']) && is_numeric($_POST['contest_id'])){//若是竞赛
				...
				if($privilege === TRUE){
					$this->oj_model->add_solution(array('problemId'=>$problemId,
														'userId'=>$data['user']['userId'],
														'programLan'=>$programLan,
														'inDate'=>mdate($now),
														'contestId'=>$contestId,
														'codeLenth'=>$code_len));
				...
				&& isset($_POST['unit_id']) && is_numeric($_POST['unit_id'])){
				//如果是课程的话
